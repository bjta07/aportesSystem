...
export default function AfiliadosPage (){
    ...
    const handleSave = async(updateMemberData) => {
        ...
        const savePromise = new Promise (async (resolve, reject) => {
            try {
                ...
                if (response.ok) {
                    setMembers(prevMembers => prevMembers.map(member => member.id_afiliado === memberId ? { ...member, ...updateMemberData } : member
                    ))
                ...
                reject(new Error ('Error de conexion al servidor'))
            }
        })
    ...
    const fetchMembers = async () => {
        try {
            setLoading(true)
            const response = await memberApi.getByCity(user.id_colegio)
    ...
    return(
        <div className={styles.tableContainer}>
            <h2>Lista de afiliados {user.nombre_colegio}</h2>
